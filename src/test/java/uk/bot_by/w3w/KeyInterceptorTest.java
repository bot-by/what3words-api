...
class KeyInterceptorTest {
	...
	void happyPath() {
		...
		assertAll("HTTP header with API key",
				() -> assertThat("HTTP header with API key", requestTemplate.headers(), hasKey("X-Api-Key")),
	...
	void queryParameterExists() {
		...
		// then
		assertAll("HTTP header with API key",
				() -> assertThat("HTTP header with API key", requestTemplate.headers(), not(hasKey("X-Api-Key"))));
	}
