...
class What3WordsErrorDecoderTest {
  ...
  void setUp() {
    ...
    response = Response.builder().request(
        ...
        .status(400).reason("Bad Request").headers(Collections.emptyMap()).body(body).build();
  }
  ...
  public void bodyIsNull() {
    ...
    response = Response.builder().request(
            Request.create(Request.HttpMethod.GET, "/api", Collections.emptyMap(), null, UTF_8, null))
        .status(500).reason("Test Error Response").headers(Collections.emptyMap()).body(body)
        .build();
    ...
    assertAll("Response has no body",
        ...
        () -> assertEquals("[500 Test Error Response] during [GET] to [/api] [qwerty]: []",
  ...
  public void unknownResponse() throws IOException {
    // given
    String bodyText = "xyz";
    when(body.asInputStream()).thenReturn(
        new ByteArrayInputStream(bodyText.getBytes())); // internal of Feign
    ...
    assertAll("Response body is not JSON",
        () -> assertThat("class", exception.getClass(), typeCompatibleWith(FeignException.class)),
        () -> assertEquals("[400 Bad Request] during [GET] to [/api] [qwerty]: [xyz]",
  ...
  public void responseIsNotError() throws IOException {
    ...
    assertAll("Response body is not error",
        () -> assertThat("class", exception.getClass(), typeCompatibleWith(FeignException.class)),
        () -> assertEquals(
            "[400 Bad Request] during [GET] to [/api] [qwerty]: [{\"warning\":\"xyz\"}]",
            exception.getMessage(), "message"));
  }
  ...
  public void decode() throws IOException {
    ...
    // then
    assertAll("Response body is not error", () -> assertThat("class", exception.getClass(),
            typeCompatibleWith(What3WordsException.class)),
